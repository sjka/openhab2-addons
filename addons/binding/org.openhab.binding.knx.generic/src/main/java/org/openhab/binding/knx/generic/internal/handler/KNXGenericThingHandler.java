...
public class KNXGenericThingHandler extends BaseThingHandler
    ...
    public void initialize() {
        ...
        knxScheduler = getMainBridgeHandler().getScheduler();
        ...
        getMainBridgeHandler().registerGroupAddressListener(this);

        scheduleReadJobs();
    }

    private KNXBridgeBaseThingHandler getMainBridgeHandler() {
        Thing bridge = getBridge();
        while (!(bridge.getHandler() instanceof KNXBridgeBaseThingHandler)) {
            bridge = thingRegistry.get(bridge.getBridgeUID());
        }
        return (KNXBridgeBaseThingHandler) bridge.getHandler();
    }
    ...
    public void bridgeStatusChanged(ThingStatusInfo bridgeStatusInfo) {
        ...
        if (bridgeStatusInfo.getStatus() == ThingStatus.ONLINE) {
            ...
            cancelReadFutures();
            getMainBridgeHandler().unregisterGroupAddressListener(this);
    ...
    public void handleRemoval() {
        ...
        KNXBridgeBaseThingHandler bridgeHandler = getMainBridgeHandler();
                            ...
                            if (channelConfiguration != null) {
                                ...
                                if (convertedType != null) {
                                    for (GroupAddress address : knxChannelSelectorProxy.getWriteAddresses(selector,
                                        ...
                                        getMainBridgeHandler().writeToKNX(address, knxChannelSelectorProxy.getDPT(
                                                address, selector, channelConfiguration, convertedType), convertedType);
                                    }
                                }
                            } else {
    ...
    public void restart() {
        if (address != null) {
            getMainBridgeHandler().restartNetworkDevice(address);
        }
    }
        ...
        public void run() {
            try {
                if (getThing().getStatus() == ThingStatus.ONLINE && getBridge().getStatus() == ThingStatus.ONLINE) {
                    ...
                    if (!getMainBridgeHandler().isDPTSupported(dpt)) {
                        logger.warn("DPT '{}' is not supported by the KNX binding", dpt);
                        return;
                    }

                    Datapoint datapoint = new CommandDP(address, getThing().getUID().toString(), 0, dpt);
                    getMainBridgeHandler().readDatapoint(datapoint, getMainBridgeHandler().getReadRetriesLimit());
                }
            } catch (Exception e) {
                logger.error(
        ...
        public void run() {
            try {
                if (address != null && getBridge().getStatus() == ThingStatus.ONLINE) {
                    ...
                    boolean isReachable = getMainBridgeHandler().isReachable(address);
                    if (isReachable) {
                        updateStatus(ThingStatus.ONLINE);
        ...
        public void run() {
            try {
                if (getBridge().getStatus() == ThingStatus.ONLINE) {
                    ...
                    byte[] data = getMainBridgeHandler().readDeviceDescription(address, 0, false, OPERATION_TIMEOUT);
                    ...
                    Thread.sleep(OPERATION_INTERVAL);
                    byte[] elements = getMainBridgeHandler().readDeviceProperties(address, DEVICE_OBJECT,
                            PID.OBJECT_TYPE, 0, 1, false, OPERATION_TIMEOUT);
                    if ((elements == null ? 0 : toUnsigned(elements)) == 1) {

                        Thread.sleep(OPERATION_INTERVAL);
                        String ManufacturerID = Manufacturer
                                .getName(toUnsigned(getMainBridgeHandler().readDeviceProperties(address, DEVICE_OBJECT,
                                        PID.MANUFACTURER_ID, 1, 1, false, OPERATION_TIMEOUT)));
                        Thread.sleep(OPERATION_INTERVAL);
                        String serialNo = DataUnitBuilder.toHex(getMainBridgeHandler().readDeviceProperties(address,
                                DEVICE_OBJECT, PID.SERIAL_NUMBER, 1, 1, false, OPERATION_TIMEOUT), "");
                        Thread.sleep(OPERATION_INTERVAL);
                        String hardwareType = DataUnitBuilder.toHex(getMainBridgeHandler().readDeviceProperties(address,
                                DEVICE_OBJECT, HARDWARE_TYPE, 1, 1, false, OPERATION_TIMEOUT), " ");
                        Thread.sleep(OPERATION_INTERVAL);
                        String firmwareRevision = Integer
                                .toString(toUnsigned(getMainBridgeHandler().readDeviceProperties(address, DEVICE_OBJECT,
                    ...
                    Thread.sleep(OPERATION_INTERVAL);
                    byte[] tableaddress = getMainBridgeHandler().readDeviceProperties(address, ADDRESS_TABLE_OBJECT,
                            PID.TABLE_REFERENCE, 1, 1, false, OPERATION_TIMEOUT);

                    if (tableaddress != null) {
                        Thread.sleep(OPERATION_INTERVAL);
                        elements = getMainBridgeHandler().readDeviceMemory(address, toUnsigned(tableaddress), 1, false,
                                OPERATION_TIMEOUT);
                        if (elements != null) {
                            ...
                            while (addressData == null) {
                                Thread.sleep(OPERATION_INTERVAL);
                                addressData = getMainBridgeHandler().readDeviceMemory(address,
                                        toUnsigned(tableaddress) + 1, 2, false, OPERATION_TIMEOUT);
                                if (addressData != null) {
                    ...
                    Thread.sleep(OPERATION_INTERVAL);
                    byte[] objecttableaddress = getMainBridgeHandler().readDeviceProperties(address, GROUPOBJECT_OBJECT,
                            PID.TABLE_REFERENCE, 1, 1, true, OPERATION_TIMEOUT);

                    if (objecttableaddress != null) {
                        Thread.sleep(OPERATION_INTERVAL);
                        elements = getMainBridgeHandler().readDeviceMemory(address, toUnsigned(objecttableaddress), 1,
                        ...
                        if (elements != null) {
                            ...
                            for (int i = 1; i < numberOfElements; i++) {
                                ...
                                while (objectData == null) {
                                    Thread.sleep(OPERATION_INTERVAL);
                                    objectData = getMainBridgeHandler().readDeviceMemory(address,
